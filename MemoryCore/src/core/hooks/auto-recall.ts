...
const RECALL_TRUNCATION_SUFFIX = "… (truncated; see tdai_memory_search or tdai_conversation_search for details)";
...
- **tdai_memory_search**: searches structured memories (L1); use it to recall key information such as user preferences, past event milestones and rules.
- **tdai_conversation_search**: searches the raw conversation (L0); use it to find the original text of specific messages, timelines and contextual details; it can also supplement or verify memory_search results.
- **read_file** (paths in Scene Navigation): use it once you have located the relevant scene and need its full profile, course of events or stage conclusions.
...
- If 3 searches still return nothing, the information is not in memory; answer the user directly from what you already have and stop searching.
</memory-tools-guide>`
...
async function performAutoRecallCore(params: {
  ...
  if (!userText || userText.length === 0) {
    ...
    recalledL1Memories = memoryLines.map((line, i) => {
      const match = line.match(/^-\s+\[([^\]]+)\]\s+(.+?)(?:\s*\((?:activity time|活动时间):.*\))?$/);
      if (match) {
        const tag = match[1];
  ...
  if (memoryLines.length > 0) {
    prependContext =
      `<relevant-memories>\nRelevant memories recalled for this conversation. They do not reflect the progress of the current task; use them for reference only:\n\n${memoryLines.join(RECALL_LINE_SEPARATOR)}\n</relevant-memories>`;
  }
