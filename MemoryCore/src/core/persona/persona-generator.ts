  ...
  async generateLocalPersona(triggerReason?: string): Promise<boolean> {
    ...
    if (changedSceneContents.length > 0) {
      changedScenesContent =
        `\n\n## 📄 Full content of the changed scenes\n\n` +
        `*These ${changedSceneContents.length} scenes changed since the last persona update. Their full content is preloaded:*\n\n` +
        changedSceneContents.join("\n\n") +
        `\n\n---\n\n` +
        `⚠️ **Focus on the changed scenes**: they are **new or modified since the last update**; analyze the new information in them.\n`;
    } else {
      changedScenesContent = `\n\n⚠️ **No changed scenes**: every scene was analyzed in the last persona update; this run may read all scenes for a global review.\n`;
    }
